...
def load_excel(url: str) -> pd.DataFrame:
    ...
    for i in range(min(10, len(raw_df))):
        ...
        if "Industrial process" in row_values:
            header_row_idx = i
            break
    ...
    df.columns = raw_df.iloc[header_row_idx].map(normalize_colname)
...
def prepare_bar_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_work[value_col] = to_numeric_safe(df_work[value_col]).fillna(0)
...
def build_fact_sheet(df: pd.DataFrame, selected_process: str):
    ...
    production_col = "Annual production in 2022 based on FU"
    annual_energy_col = "Annual energy demand in 2022"
    elec_col = "SEC electricity"
    fuel_col = "SEC fuels"
    steam_col = "SEC fuels or electricity for steam or steam from CHP"
    ...
    with left_col:
        ...
        if fact_sheet:
            ...
            if donut_fig is not None:
                ...
                st.info("No positive SEC values available for the selected process.")
